...
| VNL-060 | **`update_note` cannot change frontmatter**, so no agent can set `status: superseded` / `superseded_by` — the one signal `recall` and VNL-056 surface as "this note is outdated" can only be written by hand in Obsidian. Found 2026-09-03 while verifying `recall` live: the Paid-From-Launch note records its own supersession in prose *and* says the frontmatter could not be updated through MCP. Needs a frontmatter-merge path that keeps VNL-003's verbatim-preservation guarantee for every key it is not explicitly changing. | 4 h | todo |
