...
| ID | Item | Effort | Status |
|---|---|---|---|
| VNL-050 | **`recall(query, topK?, context?)` tool.** One call: lexical scoring (BM25 over the existing content index) → seed set → spreading activation over the weighted graph to expand and re-rank → top-K with `why` (matched terms, hop path, weight, staleness, `supersededBy`) and a **snippet** per hit so the agent does not need N `read_note` round trips. Existing tools stay for compatibility; `recall` becomes the tool description's recommended entry point. | 3 days | todo |
