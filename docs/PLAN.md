...
- **IDs**: migrated items keep their `AIBRAIN-nn` key as a stable identifier. New items are `VNL-nnn`. Next free: `VNL-061`.
...
| AIBRAIN-57 / AIBRAIN-100 | Submit to `obsidian-releases` with the required README disclosure (no telemetry, local files only, what `.vault-neural-links/` contains — including VNL-052's navigation logging, which is local-file-only but is still the user's browsing history and must be described as such). | 2 h + their queue | todo |
...
| ID | Item | Effort | Status |
|---|---|---|---|
| VNL-050 | **`recall(query, topK?, context?)` tool.** One call: lexical scoring (BM25 over the existing content index) → seed set → spreading activation over the weighted graph to expand and re-rank → top-K with `why` (matched terms, hop path, weight, staleness, `supersededBy`) and a **snippet** per hit so the agent does not need N `read_note` round trips. Existing tools stay for compatibility; `recall` becomes the tool description's recommended entry point. Done 2026-09-03: BM25 (df from the content index, tf computed live over a capped candidate set), per-seed spreading activation with the origin keeping its own activation, normalized blend at `graphWeight` 0.5, snippets, and `why`. 17 new tests (14 core, 3 mcp-server). | 3 days | done |
| VNL-060 | **`update_note` cannot change frontmatter**, so no agent can set `status: superseded` / `superseded_by` — the one signal `recall` and VNL-056 surface as "this note is outdated" can only be written by hand in Obsidian. Found 2026-09-03 while verifying `recall` live: the Paid-From-Launch note records its own supersession in prose *and* says the frontmatter could not be updated through MCP. Needs a frontmatter-merge path that keeps VNL-003's verbatim-preservation guarantee for every key it is not explicitly changing. Done 2026-09-04: `mergeFrontmatterRaw` edits the raw block line by line instead of re-serializing it, so every key the patch does not name — plus comments, key order and block-sequence style — stays byte-identical; `update_note` gained an optional `frontmatter` patch (`null` removes a key) and reports `frontmatterChanged`. A value containing a line break is refused rather than written as a broken block. 12 new tests (11 core, 1 mcp-server + 1 regression for the no-patch path). | 4 h | done |
...
| VNL-052 | **Human navigation as signal (plugin).** The Obsidian plugin logs `file-open` and `modify` events and consecutive-open pairs into the same `events/*.jsonl` log with `trigger: "human-open" / "human-edit"`, at a lower weight delta than agent traversal. Co-open within a 10-minute window → traversal edge. This is the single biggest lever against signal starvation and turns the free plugin into the thing that makes the MCP server smart. Done 2026-09-03: `HumanNavigationTracker` in core (pure, 13 tests) + `HumanActivityWatcher` in the plugin; consecutive opens within 10 min → `traverse` @ 0.25 `trigger: "human-open"`, editing the note you navigated to → `reinforce` @ 0.5 `trigger: "human-edit"`; per-edge throttles; same event log, so compaction is unchanged; usage report counts human signal on its own axis; settings toggle + README disclosure. | 2 days | done |
| VNL-053 | **Term-to-note learning.** Search queries are already logged; when a `search_notes`/`recall` result is read next, persist a `query-token → note` weight (same decay/consolidation math as note edges). The graph then learns what "kill process by port" means for *this* user, which no static ranking can. Done 2026-09-03: `term-weights.json`, a separate file from `link-weights.json` so a token can never become a graph neighbor; compactor shares its fold logic between the two graphs; `recall` blends live-decayed term scores in at `termWeight` 0.4, adding a `"term"` source for a note that surfaces on learned association alone; `search_notes`/`recall` queue credit for whichever hits are actually read next, same lifecycle as VNL-050's graph-hit crediting. Found and fixed in the same pass: term events were falling into `computeUsageReport`'s `topTouchedNotes` keyed by query token instead of note path — now counted on their own axis (`mechanismCounts.termLearn`) and excluded from touch counts entirely. Usage report panel shows both new axes. 21 new tests (18 core, 3 mcp-server). | 2 days | done |
| VNL-054 | **"Referenced" signal via write-back.** When the agent's `create_note`/`update_note` body contains `[[X]]` and X was read in this session, log a `reinforce` with `trigger: "cited"`. This is the deterministic, MCP-visible half of AIBRAIN-134's *Referenced* state; no gateway needed. Done 2026-09-04: `citedNotes` in core (pure, 8 tests) resolves a written body's wikilinks against the notes read this session only — exact path or unambiguous bare title, same resolution discipline as `buildStructuralIndex` — and mcp-server credits each new pair once per session at `CITED_REINFORCE_BOOST` (1.5, level with the removed explicit tool's boost and above auto-retrieval's 1). Only the text the agent supplied in the call is scanned, never the file as written, so the auto-linker's own "Related" wikilinks can't be mistaken for the agent citing something; `create_note`/`update_note` return the `cited` list. Usage report gained a third reinforcement axis, and the plugin panel the `termLearn` line VNL-053's entry claimed but never shipped. | 0.5 day | done |
...
- **2026-09-04 (Phase 2b, out of order)** — VNL-060 `done`: `update_note` can now change frontmatter a key at a time, so an agent can finally mark a note superseded — the one "this note is outdated" signal `recall` already surfaces, and which until now only a human editing Obsidian by hand could write. The implementation edits the raw frontmatter block line by line rather than patching the parsed object and re-serializing it, because re-serializing is exactly what VNL-003 was fixed to stop: the minimal writer in `frontmatter.ts` cannot reproduce comments, key order, block-sequence style, or any YAML it does not parse, and a note that survived a body-only edit must survive a frontmatter edit too. Only the lines of the named keys move; a `null` removes a key with its indented block; an array replacing a block sequence stays a block sequence rather than flipping to inline. One deliberate hard failure: a value containing a line break is refused with a message the model can act on, because this writer has no block-scalar syntax and the caller is now an LLM — a silently mangled frontmatter block is the kind of corruption a user would not notice for weeks. Found while writing the tests: a comment line between two keys was being swallowed into the preceding key's span and deleted on replacement — a span now ends at its key's last content line, with a regression test. 12 new tests; 313 core + 51 mcp-server tests green, all five packages typecheck, root build and the plugin bundle build.

- **2026-09-04 (Phase 2b, fourth item)** — VNL-054 `done`: the "Referenced" write-back signal. AIBRAIN-134's taxonomy says an MCP server cannot know which retrieved notes a model's answer actually used, because the protocol never shows the server that answer — true, and the reason true attribution is filed behind a model-API gateway. But there is one case it *can* see: the agent read note X this session and then wrote `[[X]]` into a note of its own. That is a citation in the tool call itself, needing neither a gateway nor a voluntary "I used this" tool call of the kind `reinforce_link` proved agents never make. It now logs a `reinforce` at 1.5 with `trigger: "cited"` — above auto-retrieval's 1, because reading a result only shows the agent looked, while citing it shows the note reached the work product; not above the old explicit boost, because AIBRAIN-66 measured how little it takes for oversized boosts to pin an arbitrary note at rank 1. Three deliberate narrowings: only notes actually **read** this session are candidates (a link to a note the agent never opened is structure, which the structural graph already covers), only the text supplied in *this* call is scanned (`writeNoteWithAutoLink` appends its own "Related (auto-linked)" wikilinks afterwards, and crediting machine-generated links as agent reasoning would be exactly the fetched-vs-useful conflation this ticket exists to avoid), and each note pair is credited once per session however often the note is rewritten. A bare `[[Title]]` resolves only when unambiguous among the notes read, mirroring `buildStructuralIndex`. Found while wiring the report: the plugin's Usage Report panel never got VNL-053's `termLearn` line despite that entry claiming it — added here alongside the new `cited` count. 8 new core tests, 3 new mcp-server tests; 301 core + 49 mcp-server tests green, all five packages typecheck, root build and the plugin bundle build.

- **2026-09-03 (Phase 2b, third item)** — VNL-053 `done`: term-to-note learning. Search queries were already logged, but the query itself — the fact that for *this* user "kill process by port" means one specific note — was thrown away; no static ranking can know that. `search_notes`/`recall` now queue credit for whichever of their hits gets actually read next (same retrieval-then-read lifecycle as AUTO_REINFORCE_BOOST), and that read persists a `query-token → note` edge per selective term into a new `term-weights.json`. Kept as a separate file from `link-weights.json` on purpose: a token is not a note, and letting one appear as a graph neighbor would let spreading activation walk into a shared word and back out into every note that ever matched it. The compactor's fold arithmetic is now shared between the two graphs rather than duplicated. `recall` blends live-decayed term scores into its ranking at `termWeight` 0.4 — below `graphWeight`'s 0.5, since a learned personal association is real evidence but easily stale and must not override what the query's words plainly say today — and adds a `"term"` source label for a hit that surfaces on that association alone, no text or graph link at all. Caught in the same pass: term events were leaking into `computeUsageReport`'s `topTouchedNotes`, keyed by the query token instead of a note path, because the touch-count loop ran unconditionally over every event type; term events are now counted on their own axis (`mechanismCounts.termLearn`) and excluded from touch counts, with a regression test. The Obsidian plugin's Usage Report panel gained both this and VNL-052's human-navigation line, which had shipped without one. 21 new tests (18 core, 3 mcp-server); 293 core + 46 mcp-server tests green, all five packages typecheck, root build succeeds.

- **2026-09-03 (Phase 2b, second item)** — VNL-052 `done`: the Obsidian plugin is now the engine's primary sensor. Two consecutive note opens within 10 minutes log a `traverse` at weight 0.25 (`trigger: "human-open"`); editing the note you navigated to logs a `reinforce` at 0.5 (`trigger: "human-edit"`), both far below an agent traversal's 1 because the human generates on the order of 100x the events and at parity the graph would become a heat map of clicks. Written to the same `events/*.jsonl` log the MCP server uses, so compaction, decay and consolidation needed no changes at all; the usage report counts human signal on its own axis so it can't be mistaken for agent traffic. Guards against counting writes the user didn't make: an edit is credited only for the currently-open note and only while the Obsidian window has focus, since the MCP server's own `create_note`/`update_note` and any file-sync client fire Obsidian's `modify` event exactly like a keystroke does. Per-edge throttles (60 s for opens, 5 min for edits) keep flipping between two notes, or saving repeatedly while writing, from counting as many relationships. The decision logic lives in core's `HumanNavigationTracker` so it is unit-testable — the plugin class is a thin Obsidian adapter — which is also the first test coverage in this repo for plugin behaviour (gap #12). Default **on**, with a settings toggle and a README disclosure: it records note paths and timestamps to a folder inside the user's own vault, never any content and never over the network, so it does not touch the opt-in rule in the telemetry decision (that governs phoning home). 288 core + 43 mcp-server tests green, all five packages typecheck, plugin bundle builds.

- **2026-09-03 (Phase 2b starts)** — VNL-050 `done`: the `recall` tool ships, so the engine's entry point is a *query*, not a note (D10). Lexical relevance (BM25; document frequencies come free from the existing content index, term frequencies are computed live over a capped candidate set — the index carries no per-note counts and giving it some belongs to VNL-031, not here) picks the seed set; spreading activation runs per seed with energy proportional to that seed's share of the lexical score; both axes are normalized within the result set and blended at `graphWeight` 0.5, so the graph re-ranks and expands but cannot outrank a strong textual match. Each hit carries a snippet and a `why` (matched terms, seed + hop count, energy, `staleDays`, `supersededBy`, `primed`). Reading a graph-expanded hit auto-reinforces the seed → hit edge, which search_notes could never do (a text query has no origin note). One defect was found by running it against the real 474-note vault and is now a regression test: a seed got no graph score of its own while its neighbours did, so a hub MOC one hop away outranked the note that answered the query; the origin now keeps its own activation. 305 tests green (262 core, 43 mcp-server), all five packages typecheck, root build succeeds. Not yet a quality *claim*: the MRR-vs-plain-search gate is VNL-020's job (D6).
