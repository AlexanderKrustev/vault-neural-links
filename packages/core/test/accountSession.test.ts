...
describe("readAccountSession", () => {
  ...
  it("round-trips a written session", async () => {
    const session = { email: "demo@vaultneurallinks.dev", plan: "demo", loggedInAt: "2026-08-18T12:00:00.000Z" };
  ...
  it("creates the parent directory on first write", async () => {
    ...
    await writeAccountSession(nestedPath, { email: "a@b.com", plan: "demo", loggedInAt: "2026-08-18T12:00:00.000Z" });
...
describe("clearAccountSession", () => {
  it("removes an existing session file", async () => {
    await writeAccountSession(path, { email: "a@b.com", plan: "demo", loggedInAt: "2026-08-18T12:00:00.000Z" });
